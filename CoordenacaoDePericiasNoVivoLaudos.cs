...
{
    public partial class CoordenacaoDePericiasNoVivoLaudosForm : Form
    {
        ...
        public CoordenacaoDePericiasNoVivoLaudosForm(IConfigurationRoot configuration, HttpClient _httpClient)
        {
        ...
        private void ButtonSelectFolder_Click(object sender, EventArgs e)
        {
            if (folderBrowserDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                Regex laudoSiplOuLaudoVivoLoteNovoNamePattern = new Regex(@"^[0-9]{4}\-[0-9]+\=[a-zA-Z·¡È…ÌÕÛ”˙⁄‡¿Ë»ÏÃÚ“˘Ÿ‚¬Í ÓŒÙ‘˚€„√ı’Á«']+\.pdf$", RegexOptions.Compiled);

                foreach (var filePath in filePaths)
                {
                    var fileName = Path.GetFileName(filePath);

                    if (laudoVivoNamePattern.IsMatch(fileName) || laudoSiplOuLaudoVivoLoteNovoNamePattern.IsMatch(fileName))
                    {
                ...
                foreach (string filePath in listFilePaths.Items)
                {
                    if (fileCounter < (int)maxFilesAsDouble)
                    {
                        ...
                        switch(comboReportType.SelectedItem!.ToString()!) //N„o pode ser por reportType porque ele È alterado no laudo morto sipl
                        {
                            ...
                            default:
                                {
                                    // Caso dos laudos vivos do novo lote. Mesma modalidade, mas usando ano + nro de laudo em vez de ano + nro protocolo.
                                    // <ano>-<nro de laudo>=<primeiro nome da pessoa>.pdf
                                    reportType = "PERÕCIA NO VIVO"; // No mais idÍntico ao SIPL, mas sem usar outra modalidade e com o ano
                                    var nameTokens = fileNameWithoutExtension.Split(['-', '=']);
                                    nomeCidadao = nameTokens[2].ToUpper();
                                    protocolo = String.Empty;
                                    noLaudo = nameTokens[1];
                        ...
                        // Tipo documental: CoordenaÁ„o de perÌcias no vivo (CPV) laudos
