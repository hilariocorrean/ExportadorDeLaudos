...
{
    public partial class PCEPACastanhalIMOLDigitalizacoesForm : Form
    {
        ...
        public PCEPACastanhalIMOLDigitalizacoesForm(IConfigurationRoot configuration, HttpClient _httpClient)
        {
        ...
        private List<OrbisDocKeywordVersion> GetOrbisDocKeywordVersionsList(string modalidadeIMOLDigitalizacoes, string nome, double? ano)
        {
            var keywordVersionList = new List<OrbisDocKeywordVersion>();

            var modalidadeIMOLDigitalizacoesKeyword = new OrbisDocKeywordVersion
            {
                KeywordTypeOf = "01998160-de9d-72d3-9711-e0414ad6078c",
                Enable = 1,
                Status = 1,
                Value = modalidadeIMOLDigitalizacoes
            };            
            var nomeKeyword = new OrbisDocKeywordVersion
            {
                KeywordTypeOf = "01998160-de9d-7bca-8c64-e6b1f5db0591",
                Enable = 1,
                Status = 1,
                Value = nome
            };
            ...
            {
                KeywordTypeOf = "01998160-de9d-7cb6-a693-5ac58ac88fab",
                Enable = 1,
                Status = 1,
                Value = ano is null ? String.Empty : ((int)ano!).ToString()
            };

            keywordVersionList.Add(modalidadeIMOLDigitalizacoesKeyword);
            ...
            try
            {
                ...
                foreach (string filePath in listFilePaths.Items)
                {
                    if (fileCounter < (int)maxFilesAsDouble)
                    {
                        ...
                        // Tipo documental: PCEPA CASTANHAL (IMOL) DIGITALIZACOES
                        var typeOf = "0199815a-99c4-7ce9-96da-f2d36d024326";
